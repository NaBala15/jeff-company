...
(function () {
  ...
  document.addEventListener('click', function (evento) {
    ...
    var posicao = alvo.getBoundingClientRect().top + window.pageYOffset - alturaCabecalho - 8;

    window.scrollTo({
      top: posicao < 0 ? 0 : posicao,
      behavior: 'smooth'
    });

    // Mantém a acessibilidade: move o foco para a seção de destino
    alvo.setAttribute('tabindex', '-1');
    alvo.focus({ preventScroll: true });
  });

  /* ---------- 3) APARIÇÃO AO ROLAR ---------- */
  var alvosAnimados = document.querySelectorAll('.secao, .hero__moldura');
